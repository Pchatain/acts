...
FW::CsvSpacePointReader::CsvSpacePointReader(
    const FW::CsvSpacePointReader::Config& cfg, Acts::Logging::Level lvl)
    : m_cfg(cfg)
  ...
  if (m_cfg.outputSpacePoints.empty()) {
    throw std::invalid_argument("Missing space points output collection");
  }
...
std::string FW::CsvSpacePointReader::CsvSpacePointReader::name() const {
  return "CsvSpacePointReader";
}

std::pair<size_t, size_t> FW::CsvSpacePointReader::availableEvents() const {
...
//   auto cells =
//       readEverything<FW::CellData>(inputDir, "cells.csv", {"timestamp"},
//       event);
...
FW::ProcessCode FW::CsvSpacePointReader::read(const FW::AlgorithmContext& ctx) {
  ...
  for (const HitData& hit : hits) {
    ...
    // Get truth particle ID. store in particleId the barcode.
    auto prtIndex = std::lower_bound(truths.begin(), truths.end(), hit.hit_id,
                                     CompareHitId{});
    particleHitCount.push_back({(*prtIndex).particle_id, 1u});
    // SpacePoint* SP =
    //     new SpacePoint{hit.hit_id, hit.x,     hit.y,     hit.z, hit.r,
    //                    hit.geoId,  varianceR, varianceZ, particleHitCount};
    // spVec.emplace_back(*SP);
    spVec.emplace_back(SpacePoint{hit.hit_id, hit.x, hit.y, hit.z,
                                  std::sqrt(hit.x * hit.x + hit.y * hit.y),
                                  hit.geometry_id, varianceR, varianceZ,
                                  particleHitCount});
    hitParticlesMap.emplace_hint(hitParticlesMap.end(), hit.hit_id,
                                 (*prtIndex).particle_id);
  }
  // std::cout << "We put in " << spVec.size() << " space points and there were
  // "
  //           << truths.size() << " truth particles" << std::endl;

  // write the data to the EventStore
  GeometryIdMultimap<Acts::PlanarModuleCluster> clusters;
  ctx.eventStore.add(m_cfg.outputClusters, std::move(clusters));
  ctx.eventStore.add(m_cfg.outputSpacePoints, std::move(spVec));
...
//   }
//   if (m_cfg.outputHitParticlesMap.empty()) {
//     throw std::invalid_argument("Missing hit-particles map output
//     collection");
//   }
//   if (m_cfg.outputSimulatedHits.empty()) {
//     throw std::invalid_argument("Missing simulated hits output collection");
...
// /// Convert separate volume/layer/module id into a single geometry
// identifier. inline Acts::GeometryID extractGeometryId(const FW::HitData&
// data) {
//   // if available, use the encoded geometry directly
//   if (data.geometry_id != 0u) {
//     return data.geometry_id;
...
// std::vector<FW::TruthHitData> readTruthHitsByHitId(const std::string&
// inputDir,
//                                                    size_t event) {
//   // define all optional columns
//   std::vector<std::string> optionalColumns = {
...
//         // hits by geometry id should ensure that new sim hits are always
//         added
...
//           ACTS_FATAL("Truth hit sorting broke for input hit id " <<
//           hit.hit_id); return ProcessCode::ABORT;
//         }
...
//         std::move(cov), local[0], local[1], time,
...
//     auto truthRange = makeRange(std::equal_range(truths.begin(),
//     truths.end(),
//                                                  hit.hit_id,
//                                                  CompareHitId{}));
...
//   ctx.eventStore.add(m_cfg.outputClusters, std::move(clusters));
//   ctx.eventStore.add(m_cfg.outputHitIds, std::move(hitIds));
//   ctx.eventStore.add(m_cfg.outputHitParticlesMap,
//   std::move(hitParticlesMap)); ctx.eventStore.add(m_cfg.outputSimulatedHits,
//   std::move(simHits));
